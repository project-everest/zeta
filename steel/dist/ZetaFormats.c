...
{
  uint32_t len1 = u256_lserializer(x.raw_key_k, input, pos);
  uint32_t pos1 = pos + len1;
  uint32_t len2 = significant_digits_t_lserializer(x.raw_key_significant_digits, input, pos1);
  return len1 + len2;
}
...
{
  uint32_t len1 = base_key_lserializer(x.dhd_key, input, pos);
  uint32_t pos10 = pos + len1;
...
{
  return base_key_reader(input, pos);
}
