...
static uint32_t evictbm_payload_size32(evictbm_payload input)
{
  ...
  uint32_t v1;
  if ((uint32_t)4294967295U - v20 < v10)
    v1 = (uint32_t)4294967295U;
  else
    v1 = v10 + v20;
...
extern uint32_t Zeta_Formats_Aux_Base_key_base_key_size32(Zeta_Steel_KeyUtils_raw_key uu___);

extern uint64_t
Zeta_Formats_Aux_Base_key_base_key_validator(LowParse_Slice_slice x0, uint64_t x1);
...
Zeta_Formats_Aux_Base_key_base_key_reader(LowParse_Slice_slice x0, uint32_t x1);

extern uint32_t
Zeta_Formats_Aux_Base_key_base_key_lserializer(
  Zeta_Steel_KeyUtils_raw_key uu___,
  uint8_t *x0,
  uint32_t x1
);
...
static uint32_t u256_size32(u256 input)
{
  ...
  uint32_t v21 = (uint32_t)8U;
  uint32_t v2;
  if ((uint32_t)4294967295U - v21 < v11)
    v2 = (uint32_t)4294967295U;
  else
    v2 = v11 + v21;
  if ((uint32_t)4294967295U - v2 < v1)
    return (uint32_t)4294967295U;
...
static uint32_t u256_lserializer(u256 x, uint8_t *input, uint32_t pos)
{
  ...
  uint32_t len1 = (uint32_t)8U;
  uint32_t pos10 = pos + len1;
  store64_be(input + pos10, x.v2);
  uint32_t len2 = (uint32_t)8U;
  uint32_t len10 = len1 + len2;
  uint32_t pos1 = pos + len10;
  store64_be(input + pos1, x.v1);
  uint32_t len11 = (uint32_t)8U;
  uint32_t pos11 = pos1 + len11;
  store64_be(input + pos11, x.v0);
  uint32_t len20 = (uint32_t)8U;
  uint32_t len21 = len11 + len20;
  return len10 + len21;
}
...
{
  uint32_t v10 = Zeta_Formats_Aux_Base_key_base_key_size32(input.dhd_key);
...
{
  uint64_t pos10 = Zeta_Formats_Aux_Base_key_base_key_validator(input, pos);
  uint64_t pos1;
  if (is_error(pos10))
    pos1 = pos10;
  else
...
{
  Zeta_Steel_KeyUtils_raw_key x1 = Zeta_Formats_Aux_Base_key_base_key_reader(input, pos);
...
{
  uint32_t len1 = Zeta_Formats_Aux_Base_key_base_key_lserializer(x.dhd_key, input, pos);
  uint32_t pos10 = pos + len1;
  uint32_t len2 = hash_value_lserializer(x.dhd_h, input, pos10);
  uint32_t len10 = len1 + len2;
  uint32_t pos1 = pos + len10;
  uint32_t len20 = vbool_lserializer(x.evicted_to_blum, input, pos1);
  return len10 + len20;
}
...
static uint32_t descendent_hash_lserializer(descendent_hash x, uint8_t *b, uint32_t pos)
{
  ...
  b[pos] = ite0;
  uint32_t len = (uint32_t)1U;
  uint32_t pos_ = pos + len;
  uint32_t len1 = pos_ - pos;
  uint32_t pos1 = pos + len1;
  uint32_t len2;
  if (Vnone == tg)
    len2 = (uint32_t)0U;
  else
  {
    ...
    if (x.tag == Dh_vsome)
      ...
      ite = KRML_EABORT(descendent_hash_desc, "unreachable (pattern matches are exhaustive in F*)");
    len2 = descendent_hash_desc_lserializer(ite, b, pos1);
  }
  return len1 + len2;
}
...
{
  uint32_t len1 = descendent_hash_lserializer(x.l, input, pos);
  uint32_t pos1 = pos + len1;
  uint32_t len2 = descendent_hash_lserializer(x.r, input, pos1);
  return len1 + len2;
}
...
static uint32_t value_lserializer(value x, uint8_t *b, uint32_t pos)
{
  ...
  if (MV == tg)
  {
    ...
    len2 = mval_value_lserializer(ite, b, pos1);
  }
  else if (DVNone == tg)
    len2 = (uint32_t)0U;
  else
  {
    ...
    len2 = application_value_lserializer(ite, b, pos1);
  }
  return len1 + len2;
}
...
{
  return Zeta_Formats_Aux_Base_key_base_key_size32(input);
}

static uint64_t internal_key_validator(LowParse_Slice_slice input, uint64_t pos)
{
  uint64_t res = Zeta_Formats_Aux_Base_key_base_key_validator(input, pos);
  if (is_error(res))
    return res;
  else
  {
    Zeta_Steel_KeyUtils_raw_key
    bk = Zeta_Formats_Aux_Base_key_base_key_reader(input, (uint32_t)pos);
...
{
  return Zeta_Formats_Aux_Base_key_base_key_reader(input, pos);
}

static uint32_t
internal_key_lserializer(Zeta_Steel_KeyUtils_raw_key x, uint8_t *input, uint32_t pos)
{
  return Zeta_Formats_Aux_Base_key_base_key_lserializer(x, input, pos);
}
...
{
  uint32_t len1 = internal_key_lserializer(x.ir_key, input, pos);
  uint32_t pos1 = pos + len1;
  uint32_t len2 = mval_value_lserializer(x.ir_value, input, pos1);
  return len1 + len2;
}
...
{
  uint32_t len1 = application_key_lserializer(x.ar_key, input, pos);
  uint32_t pos1 = pos + len1;
  uint32_t len2 = application_record_v_payload_lserializer(x.v_payload, input, pos1);
  return len1 + len2;
}
...
static uint32_t record_lserializer(record x, uint8_t *b, uint32_t pos)
{
  ...
  if (Key_internal == tg)
  {
    ...
    if (x.tag == Record_kv_key_internal)
      ...
      ite = KRML_EABORT(internal_record, "unreachable (pattern matches are exhaustive in F*)");
    len2 = internal_record_lserializer(ite, b, pos1);
  }
  else
  {
    application_record ite;
    if (x.tag == Record_kv_key_application)
      ...
      ite = KRML_EABORT(application_record, "unreachable (pattern matches are exhaustive in F*)");
    len2 = application_record_lserializer(ite, b, pos1);
  }
  return len1 + len2;
}
...
{
  uint32_t len1 = record_lserializer(x.sr_record, input, pos);
  uint32_t pos10 = pos + len1;
  uint32_t len2 = timestamp_lserializer(x.sr_timestamp, input, pos10);
  uint32_t len10 = len1 + len2;
  uint32_t pos1 = pos + len10;
  uint32_t len20 = thread_id_lserializer(x.sr_thread_id, input, pos1);
  return len10 + len20;
}
...
zeta__parser_log_entry(uint32_t len, uint32_t offset, uint32_t slice_len, uint8_t *a)
{
  ...
  else
  {
    ...
    if (uu___is_Le_payload_RunApp(hdr))
    {
      ...
      uint8_t *x0 = sl.base;
      uint32_t len_pl = load32_be(x0 + pos_pl);
