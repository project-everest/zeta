...
#push-options "--query_stats --fuel 2 --ifuel 2 --z3rlimit_factor 3"
let snoc_entries (tsm:M.thread_state_model {not tsm.failed})
                 (le:log_entry)
  : Lemma (ensures (M.verify_step_model tsm le).processed_entries ==
                    Seq.snoc tsm.processed_entries le)
  = match le with
    | RunApp pl ->
      let rec write_slots_processed_entries_framing
              (tsm:M.thread_state_model)
              (slots:Seq.seq slot_id { forall (s:U16.t). {:pattern Seq.contains slots s}
                                                Seq.contains slots s ==>
                                                M.has_slot tsm s /\
                                                ApplicationKey? (M.key_of_slot tsm s)
                               })
              (values:Seq.seq (Zeta.App.app_value_nullable aprm.adm) {
                                                     Seq.length slots == Seq.length values
                        })
      : Lemma (ensures (
                       let tsm' = M.write_slots tsm slots values in
                       tsm'.processed_entries == tsm.processed_entries))
              (decreases (Seq.length slots))
      = if Seq.length slots = 0 then ()
        else (
          ...
          let hd_value =
            ...
            | Zeta.App.Null -> DValue None
            | Zeta.App.DValue d -> DValue (Some d)
          in
          let tsm = M.update_value tsm hd_slot hd_value in
          write_slots_processed_entries_framing tsm tl (Seq.tail values)
        )
      in
      let tsm' = M.runapp tsm pl in
      if not (Map.contains aprm.tbl pl.fid)
      then ()
      else (
        ...
        | Some ((arg, slots), len) ->
          ...
          then ()
            ...
            if not (Zeta.SeqAux.distinct_elems_comp slots)
            then ()
            else (
              let fsig = Map.sel aprm.tbl pl.fid in
              match M.read_slots tsm slots with
              | None -> ()
              | Some recs ->
                if not (M.check_distinct_keys recs)
                then ()
                else (
                  let res = fsig.f arg recs in
                  match res with
                  | ( Zeta.App.Fn_failure, _, _) -> ()
                  | ( _, res, out_vals) ->
                    let tsm'' = {tsm with app_results = Seq.snoc tsm.app_results (| pl.fid, arg, recs, res |) } in
                    assert (tsm' == M.write_slots tsm'' slots out_vals);
                    assert (tsm''.processed_entries == tsm.processed_entries);
                    write_slots_processed_entries_framing tsm'' slots out_vals;
                    assert (tsm'.processed_entries == tsm''.processed_entries)
                )
            )
          )
