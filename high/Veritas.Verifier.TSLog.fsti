...
(* we never see operations on Root so its eac state is always init or a special state EACRoot
 * that we transition to when we see NextEpoch or VerifyEpoch *)
val lemma_eac_state_of_root_init (itsl: eac_log):
  Lemma (ensures (let es = eac_state_of_key itsl Root in
                  es = EACInit || es = EACRoot))

(* (Near) inverse of the previous lemma - if the eac state of a key is EACRoot, then that key is Root *)
val lemma_eac_state_root_implies_root (itsl: eac_log) (k: key):
