#include <ZetaFormats.h>
 ...
 extern uint64_t App_val_app_val_validator(LowParse_Slice_slice sl, uint64_t pos);

extern uint32_t App_val_app_val_jumper(LowParse_Slice_slice input, uint32_t pos);

extern uint64_t App_val_app_val_reader(LowParse_Slice_slice sl, uint32_t pos);

extern uint32_t App_val_app_val_lserializer(uint64_t v, uint8_t *b, uint32_t pos);

extern uint64_t App_key_app_key_validator(LowParse_Slice_slice sl, uint64_t pos);

extern uint32_t App_key_app_key_jumper(LowParse_Slice_slice input, uint32_t pos);

extern uint64_t App_key_app_key_reader(LowParse_Slice_slice sl, uint32_t pos);

extern uint32_t App_key_app_key_lserializer(uint64_t v, uint8_t *b, uint32_t pos);
