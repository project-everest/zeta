...
let lemma_extend_rw_consistency (s:seq state_op{length s > 0}):
  Lemma (requires (rw_consistent_idx s (length s - 1) /\
...
let rec eval_rw_consistency_aux (s: seq (state_op)):
  ...
  if n = 0 then (true, 0)
    ...
    if c' then 
      if rw_consistent_idx s (n - 1) then (
        lemma_extend_rw_consistency s;
        (true, 0)
      )
      else (false, n - 1)          
    else (
      (* necessary assert *)
      assert(rw_inconsistent_idx s i');
      (false, i')
    )
