...
val lemma_ts_add_set_key_extend (itsl: its_log {length itsl > 0}):
  Lemma (requires (is_blum_add (I.telem itsl)))
        (ensures (ts_add_set_key itsl (key_of (index itsl (length itsl - 1))) == 
                  add_elem (ts_add_set_key (I.prefix itsl (length itsl - 1))
...
val lemma_ts_add_set_contains_add_elem (itsl: its_log) (i:I.seq_index itsl):
  Lemma (requires (is_blum_add (index itsl i)))
        (ensures (MS.contains (blum_add_elem (index itsl i)) (ts_add_set itsl)))
...
val some_add_elem_idx (itsl: its_log) 
  ...
  (i:(I.seq_index itsl){is_blum_add (index itsl i) /\
                      be = blum_add_elem (index itsl i)})

(* get the blum evict element from an index *)
val blum_evict_elem (itsl: its_log) (i:I.seq_index itsl{is_evict_to_blum (index itsl i)}):
  (e:ms_hashfn_dom{MH.key_of e = key_of (index itsl i)})

val lemma_index_blum_evict_prefix (itsl: its_log) (i:nat{i <= length itsl}) (j:nat{j < i}):
  Lemma (requires (is_evict_to_blum (index itsl j)))
 ...
 *)
val lemma_ts_evict_set_key_extend2 (itsl: its_log {length itsl > 0}):
  Lemma (requires (not (is_evict_to_blum (index itsl (length itsl - 1)))))
        (ensures (ts_evict_set_key itsl (key_of (index itsl (length itsl - 1))) == 
                  ts_evict_set_key (I.prefix itsl (length itsl - 1))
                                           (key_of (index itsl (length itsl - 1)))))
...
val index_blum_evict (itsl: its_log) (e: ms_hashfn_dom {contains e (ts_evict_set itsl)}):
  (i:I.seq_index itsl{is_evict_to_blum (index itsl i) /\ 
                    blum_evict_elem itsl i = e})

(* if the blum add occurs in the blum evict set, its index is earlier *)
val lemma_evict_before_add (itsl: its_log) (i:I.seq_index itsl{is_blum_add (index itsl i)}):
  Lemma (requires True)
...
val lemma_evict_before_add2 (itsl: its_log) (i:I.seq_index itsl{is_blum_add (index itsl i)}):
   Lemma (requires True)
         (ensures (MS.mem (blum_add_elem (index itsl i)) (ts_evict_set itsl) =
                   MS.mem (blum_add_elem (index itsl i)) (ts_evict_set (I.prefix itsl i))))

val lemma_evict_before_add3 (itsl: its_log) (i: I.seq_index itsl) (j:I.seq_index itsl):
  Lemma (requires (is_blum_add (index itsl i) /\
                   is_evict_to_blum (index itsl j) /\
...
val lemma_mem_monotonic (be:ms_hashfn_dom) (itsl: its_log) (i:nat{i <= length itsl}):
...
val lemma_blum_evict_add_same (itsl: TL.eac_log) (i:I.seq_index itsl):
  Lemma (requires (is_evict_to_blum (index itsl i) /\
                   TL.has_next_add_of_key itsl i (key_of (index itsl i))))
        (ensures (is_blum_add (index itsl (TL.next_add_of_key itsl i (key_of (index itsl i)))) /\
                  blum_evict_elem itsl i =                                   
                  blum_add_elem (index itsl (TL.next_add_of_key itsl i (key_of (index itsl i))))))
...
val lemma_eac_evicted_blum_implies_previous_evict (itsl: its_log) (k:key):
  Lemma (requires (is_eac_state_evicted_blum itsl k))
        (ensures (has_some_entry_of_key itsl k /\
                  is_evict_to_blum (index itsl (last_idx_of_key itsl k)) /\
