...
val lemma_ts_add_set_contains_add_elem (#n:pos) (itsl: its_log n) (i:seq_index itsl):
  Lemma (requires (is_blum_add (index itsl i)))
        (ensures (MS.contains (blum_add_elem (index itsl i)) (ts_add_set itsl)))

val lemma_ts_add_set_key_contains_only (#p:pos) (itsl: its_log p) (k:key) (be: ms_hashfn_dom):
  Lemma (requires (MS.contains be (ts_add_set_key itsl k)))
        (ensures (MH.key_of be = k))

val some_add_elem_idx (#p:pos) (itsl: its_log p) 
...
val lemma_mem_key_add_set_same (#p:pos) (itsl: its_log p) (be: ms_hashfn_dom):
  Lemma (mem be (ts_add_set itsl) = mem be (ts_add_set_key itsl (MH.key_of be)))

val lemma_mem_key_evict_set_same (#p:pos) (itsl: its_log p) (be: ms_hashfn_dom):
