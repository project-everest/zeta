...
val lemma_hprefix_append_telem (#a:Type) (s: seq a{length s > 0}):
...
(* is ps a prefix of s *)
let is_prefix (#a:Type) (s:seq a) (ps: seq a): Type0 =
  length ps <= length s /\
  ps == prefix s (length ps)

let prefix_of (#a:Type) (s0 s1:seq a) = is_prefix s1 s0
...
val proj (#a:Type u#a): seq a -> seq a -> Type u#a
...
let proj_index_map_exists (#a:Type) (ss: seq a) (s: seq a{proj ss s}) (i:seq_index ss)
...
val lemma_proj_monotonic (#a:Type) (ss s: seq a) (prf: proj ss s) (i1 i2: seq_index ss):
Lemma (requires (i1 < i2))
      (ensures (proj_index_map ss s prf i1 < proj_index_map ss s prf i2))

val lemma_proj_length (#a:Type) (ss: seq a) (s:seq a{proj ss s}):
Lemma (requires (True))
...
let filter_refine (#a:Type) (f:a -> bool) (s: seq a): Tot (seq (refine f)) =
let fs = filter f s in
seq_refine f fs

(* filter_index_map is injective *)
val lemma_filter_index_map_monotonic (#a:Type) (f:a -> bool) (s:seq a)
...
val lemma_filter_index_inv_map_monotonic (#a:Type) (f:a -> bool) (s: seq a)
...
val lemma_filter_maps_correct (#a:Type) (f:a -> bool) (s: seq a) (i:seq_index s):
  Lemma (requires (f (index s i)))
        (ensures (filter_index_map f s (filter_index_inv_map f s i) = i))

val lemma_filter_maps_correct2 (#a:Type) (f:a -> bool) (s: seq a) (i: seq_index (filter f s)):
  Lemma (requires(True))
        (ensures (filter_index_inv_map f s (filter_index_map f s i) = i))
        [SMTPat (filter_index_map f s i)]

val lemma_filter_empty (#a:Type) (f:a -> bool):
  Lemma (filter f (empty #a) == empty #a)

val lemma_filter_prefix (#a:Type) (f:a -> bool) (s: seq a) (ps: seq a{is_prefix s ps}):
  Lemma (is_prefix (filter f s) (filter f ps))

val lemma_filter_prefix_comm (#a:Type) (f:a->bool) (s: seq a) (i:seq_index s):
  Lemma (requires (f (index s i)))
...
val lemma_filter_extensionality (#a:Type) (f1 f2:a -> bool) (s:seq a):
  Lemma (requires (ext_pred f1 f2))
        (ensures (filter f1 s == filter f2 s))

let conj (#a:Type) (f1 f2: a -> bool) (x: a) =
  f1 x && f2 x

val lemma_filter_conj (#a:Type) (f1 f2: a -> bool) (s:seq a):
  Lemma (filter (conj f1 f2) s == filter f1 (filter f2 s))

val lemma_filter_comm (#a:Type) (f1 f2:a -> bool) (s:seq a):
...
let last_index (#a:Type) (f:a -> bool) (s:seq a{exists_sat_elems f s}) =
...
val lemma_last_index_correct2 (#a:Type) (f:a -> bool) (s:seq a) (i:seq_index s):
  Lemma (requires (f (index s i)))
...
val lemma_last_index_prefix (#a:Type) (f:a -> bool) (s:seq a) (i:nat{i <= length s}):
  Lemma (requires (exists_sat_elems f s /\ i > last_index f s))
...
val lemma_last_index_last_elem_sat (#a:Type) (f:a -> bool) (s:seq a{length s > 0}):
  Lemma (requires (f (index s (length s - 1))))
        (ensures (exists_sat_elems f s /\ last_index f s = length s - 1))

val lemma_exists_sat_elems_extensionality (#a:Type) (f1 f2:a -> bool) (s: seq a):
  Lemma (requires (ext_pred f1 f2))
...
val lemma_first_index_correct1 (#a:Type) (f:a -> bool) (s:seq a) (i:seq_index s):
  Lemma (requires (exists_sat_elems f s /\ i < first_index f s))
        (ensures (not (f (index s i))))

(* Any witness satisfying f implies first_index exists *)
val lemma_first_index_correct2 (#a:Type) (f:a -> bool) (s:seq a) (i:seq_index s):
  Lemma (requires (f (index s i)))
...
val zip (#a #b: Type) (sa: seq a) (sb: seq b{length sb = length sa}):
  sab: seq (a * b){length sab = length sa}

val lemma_zip_index (#a #b: Type) (sa: seq a) (sb: seq b{length sb = length sa}) (i: seq_index sa):
  Lemma (requires (True))
        (ensures (fst (index (zip sa sb) i) == index sa i /\
                  snd (index (zip sa sb) i) == index sb i))
        [SMTPat (index (zip sa sb) i)]

val unzip (#a #b: Type) (sab: seq (a * b)): sasb: (seq a * seq b)
  {length (fst sasb) = length sab /\
   length (snd sasb) = length sab}

val lemma_unzip_index (#a #b: Type) (sab: seq (a * b)) (i:seq_index sab):
  Lemma (requires (True))
...
val lemma_zip_unzip (#a #b: Type) (sa: seq a) (sb: seq b{length sb = length sa}):
  Lemma (requires (True))
        (ensures ((sa, sb) == unzip (zip sa sb)))

val lemma_unzip_extend (#a #b: Type) (sab: seq (a * b){length sab > 0}):
...
val lemma_reduce_empty (#a:Type) (#b:Type) (b0:b) (f:a -> b -> b):
...
let lemma_reduce_property_closure (#a:Type) (#b:Type) (p: b -> bool) (b0:b) (f: a -> b -> b) (s: seq a)
  : Lemma (requires (p b0 /\ (forall (x:a). forall (y:b). (p y ==> p (f x y)))))
          (ensures (p (reduce b0 f s)))
  = lemma_reduce_property_closure_ty #a #b (fun b -> p b == true) b0 f s

val lemma_reduce_identity (#a:Type) (#b:Type) (b0: b) (f: a -> b -> b) (s: seq a):
  Lemma (requires (forall (x:a). f x b0 == b0))
        (ensures (reduce b0 f s == b0))

val lemma_reduce_singleton (#a:Type) (#b:Type) (b0: b) (f: a -> b -> b) (s: seq a{length s = 1}):
...
val lemma_reduce_append2 (#a:Type) (#b:Type) (b0:b) (f: a -> b -> b) (s: seq a{length s > 0}):
...
val intro_has_next (#a:Type) (f:a -> bool) (s:seq a) (i:seq_index s) (k:seq_index s{i < k /\ f (Seq.index s k)})
...
val filter_snoc (#a:Type) (f:a -> bool) (s:seq a) (x:a)
  : Lemma (if f x 
...
val filter_map_emp (#a:Type) (#b:Type) (filter: a -> bool) (f:refine filter -> b) 
...
let distinct_elems (#a: Type) (s: seq a)
