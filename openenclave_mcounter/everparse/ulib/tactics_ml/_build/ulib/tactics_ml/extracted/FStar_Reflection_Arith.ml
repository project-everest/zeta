...
let liftM2 : 'a 'b 'c . ('a -> 'b -> 'c) -> 'a tm -> 'b tm -> 'c tm =
  fun f ->
    fun x -> fun y -> bind x (fun xx -> bind y (fun yy -> return (f xx yy)))
let liftM3 :
    ...
    fun uu___ ->
      fun s ->
        FStar_Tactics_Result.Success
          ((match uu___ with
            | (n, atoms) ->
                (match find_idx (FStar_Reflection_Builtins.term_eq t) atoms
                 with
                 | FStar_Pervasives_Native.None ->
...
let rec list_unref : 'a 'p . ('a, 'p) refined_list_t -> 'a Prims.list =
  fun l -> match l with | [] -> [] | x::xs -> x :: (list_unref xs)
let (collect_app_ref :
  FStar_Reflection_Types.term ->
    (FStar_Reflection_Types.term * (FStar_Reflection_Data.argv,
...
let rec (as_arith_expr : FStar_Reflection_Types.term -> expr tm) =
  fun t ->
    let uu___ = collect_app_ref t in
    match uu___ with
    | (hd, tl) ->
        let tl1 = list_unref tl in
        (match ((FStar_Reflection_Builtins.inspect_ln hd), tl1) with
         | (FStar_Reflection_Data.Tv_FVar fv,
            (e1, FStar_Reflection_Data.Q_Implicit)::(e2,
                                                     FStar_Reflection_Data.Q_Explicit)::
            (e3, FStar_Reflection_Data.Q_Explicit)::[]) ->
             let qn = FStar_Reflection_Builtins.inspect_fv fv in
         ...
         | (FStar_Reflection_Data.Tv_FVar fv,
            (l, FStar_Reflection_Data.Q_Explicit)::(r,
                                                    FStar_Reflection_Data.Q_Explicit)::[])
             ->
             let qn = FStar_Reflection_Builtins.inspect_fv fv in
         ...
         | (FStar_Reflection_Data.Tv_FVar fv,
            (l, FStar_Reflection_Data.Q_Implicit)::(r,
                                                    FStar_Reflection_Data.Q_Explicit)::[])
             ->
             let qn = FStar_Reflection_Builtins.inspect_fv fv in
             let ll = as_arith_expr l in
  ...
  fun t ->
    bind (as_arith_expr t)
      (fun a ->
         match a with
         | Atom (uu___, t1) ->
             let uu___1 = collect_app_ref t1 in
             (match uu___1 with
              | (hd, tl) ->
                  (match ((FStar_Reflection_Builtins.inspect_ln hd), tl) with
                   | (FStar_Reflection_Data.Tv_FVar uu___2, []) -> return a
                   | (FStar_Reflection_Data.Tv_BVar uu___2, []) -> return a
                   | (FStar_Reflection_Data.Tv_Var uu___2, []) -> return a
                   | uu___2 ->
                       fail
    ...
    fun i ->
      bind (lift FStar_Reflection_Formula.term_as_formula t)
        (fun f ->
           ...
           | FStar_Reflection_Formula.Comp
               (FStar_Reflection_Formula.BoolEq uu___, l, r) ->
               liftM2 eq (is_arith_expr l) (is_arith_expr r)
           | FStar_Reflection_Formula.Comp
               (FStar_Reflection_Formula.Lt, l, r) ->
               liftM2 lt (is_arith_expr l) (is_arith_expr r)
           | FStar_Reflection_Formula.Comp
               (FStar_Reflection_Formula.Le, l, r) ->
           ...
           | uu___ ->
               fail
                 (Prims.strcat "connector ("
                    (Prims.strcat
                       (FStar_Reflection_Builtins.term_to_string t) ")"))) i
let run_tm :
  'a .
    'a tm ->
      FStar_Tactics_Types.proofstate ->
