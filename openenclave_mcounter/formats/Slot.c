...
uint64_t Slot_slot_validator(LowParse_Slice_slice sl, uint64_t pos)
{
...
uint32_t Slot_slot_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)2U;
}

uint16_t Slot_slot_reader(LowParse_Slice_slice sl, uint32_t pos)
{
  uint8_t *x0 = sl.base;
  return load16_be(x0 + pos);
}

uint32_t Slot_slot_lserializer(uint16_t v, uint8_t *b, uint32_t pos)
{
  store16_be(b + pos, v);
  return (uint32_t)2U;
}
